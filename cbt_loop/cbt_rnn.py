...
def init_params(
    ...
    params = {
        ...
        # Cortex exc → striatum / SNc / medulla / STN / SC.
        ...
        "B_c_stn": (1 / math.sqrt(n_c_exc)) * jr.normal(skeys[26], (n_stn, n_c_exc)),  # hyperdirect pathway
        "B_c_sc": (1 / math.sqrt(n_c_exc)) * jr.normal(skeys[35], (n_sc, n_c_exc)),
        ...
        "B_snr_sc": (1 / math.sqrt(n_snr)) * jr.normal(skeys[36], (n_sc, n_snr)),  # SNr → SC (inh)
        "B_sc_t_exc": (1 / math.sqrt(n_sc)) * jr.normal(skeys[37], (n_t_exc, n_sc)),  # SC → thalamus exc
        "B_sc_t_inh": (1 / math.sqrt(n_sc)) * jr.normal(skeys[52], (n_t_inh, n_sc)),  # SC → thalamus inh
        "B_sc_med": (1 / math.sqrt(n_sc)) * jr.normal(skeys[38], (n_med // 2, n_sc)),  # SC → medulla E units
        ...
        "B_c_med": (1 / math.sqrt(n_c_exc)) * jr.normal(skeys[14], (2, n_c_exc)),  # cortex (exc) → E units only
...
def multiregion_rnn(params, config, inputs, opto_stimulation=None, rng_key=None):
    ...
    b_c_med = exc(params["B_c_med"])  # shape (2, n_c): projects to E units only
    ...
    tau_sc = config.get("tau_sc", tau_c)
    ...
    def _step(carry, inp_stim_rng):
        ...
        (rng_d1, rng_d2,
         rng_c_exc, rng_c_inh,
         rng_t_exc, rng_t_inh,
         rng_snr, rng_sc, rng_gpe, rng_stn, rng_snc, rng_med) = jr.split(step_rng, 12)
        ...
        x_t_exc = x_t_exc + (1.0 / tau_t) * tanh(b_sc_t_exc @ x_sc)
        x_t_exc = nln(x_t_exc)
        ...
        x_t_inh = x_t_inh + (1.0 / tau_t) * tanh(b_sc_t_inh @ x_sc)
        x_t_inh = nln(x_t_inh)
        ...
        x_stn = x_stn + (1.0 / tau_stn) * tanh(b_c_stn @ x_c_exc)  # hyperdirect pathway
        ...
        x_sc = x_sc + (1.0 / tau_sc) * tanh(b_c_sc @ x_c_exc)
        ...
        x_med = x_med.at[:2].add((1.0 / tau_med) * tanh(b_c_med @ x_c_exc))
