...
# plotting_functions imports the config under a different path
# (`cbt_loop.config_script`), which Python treats as a separate module from
# `config_script`. We patch both to keep `save_fig` in sync with our redirects.
_PATCH_TARGETS = [cfg]
try:
    import cbt_loop.config_script as _cfg_alt  # type: ignore  # noqa: F401
    if _cfg_alt is not cfg:
        _PATCH_TARGETS.append(_cfg_alt)
except ImportError:
    pass
