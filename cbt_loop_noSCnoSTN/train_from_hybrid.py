...
def _make_dual_cue_weights(params, rng_key):
    ...
    The cue targets all three cortical pools here (cU, cL, AND the inhibitory pool
    c_inh) — noSCnoSTN's cortex receives cue into c_inh, unlike cbt_loop — so all
    three cue tensors get the same two-column treatment.
    """
    p = dict(params)
    k_U, k_L, k_I = jr.split(rng_key, 3)
    for key, sub in (("B_cue_cU", k_U), ("B_cue_cL", k_L), ("B_cue_c_inh", k_I)):
