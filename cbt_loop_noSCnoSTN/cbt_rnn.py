...
def match_input_channels(inputs, params):
    """Zero-pad cue input channels to match the model's B_cue_c width.
...
def init_params(
    ...
    params = {
        ...
        "m_d2": (1 / math.sqrt(n_snc)) * jr.normal(skeys[11], (n_d2, n_snc)),
...
def multiregion_rnn(params, config, inputs, opto_stimulation=None, rng_key=None):
    ...
    b_t_c = exc(params["B_t_c"])
    b_c_t = exc(params["B_c_t"])
    ...
    m_d1 = m_floor + softplus(params["m_d1"])
    m_d2 = m_cap * sigmoid(params["m_d2"])
    ...
    k_a2r_raw = jnp.asarray(params["k_a2r"]) if "k_a2r" in params else jnp.asarray(config.get("k_a2r", 0.15))
